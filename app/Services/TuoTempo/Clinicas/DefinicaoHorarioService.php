...
class DefinicaoHorarioService extends BaseService {
    ...
    public function __construct(DefinicaoHorarioRepository $defRep, DefinicaoHorariosAdicionaisRepository $defAdicional, DiasAtendimentoRepository $diasAtend) {
        $this->definicaoHorarioRepository = $defRep;
        $this->definicaoHorarioAdicionalRepository = $defAdicional;
        $this->diasAtendimentoRepository = $diasAtend;
    }
...
//            sort($HORARIOS_REMOVER[$rowDiaAtendimento->dias_da_semana_id]); //orcdenando horários
            ...
            if (count($rowDiaAtendimento) > 0) {
                ...
                $DIAS_HORARIOS['nomeDia'] = $rowDiaAtendimento[0]->nomeDia;
